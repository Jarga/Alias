...
class FindExtensions {
    ...
    static FindSubElement(baseObject: webdriver.IWebElementFinders, elementProperties: { [key: string]: string }): webdriver.promise.Promise<SeleniumWebElement> {
        return baseObject.findElement(this.BuldUniversalByClause(elementProperties))
                              .then((value) => new SeleniumWebElement(value));
    }
    
    static FindSubElements(baseObject: webdriver.IWebElementFinders, elementProperties: { [key: string]: string }): webdriver.promise.Promise<SeleniumWebElement[]>{

        return (baseObject.findElements(this.BuldUniversalByClause(elementProperties))
                               .then(results => (results.length < 1 ? [] : results.map((e) => new SeleniumWebElement(e)))));
    }
