...
var FindExtensions = (function () {
    function FindExtensions() {
    }
    FindExtensions.BuildBy = function (key, value) {
    ...
    FindExtensions.BuildCompositeXPathBy = function (keyValueDictionary) {
        var keys = [];
        ...
        for (var key in keys) {
            if (keys.hasOwnProperty(key)) {
                ...
                switch (key) {
                    case "class":
                    case "classname":
                        xpathExpression += FindExtensions.GetAttributeXPath("class", value, isContains);
                        continue;
                    case "id":
                        xpathExpression += FindExtensions.GetAttributeXPath("id", value, isContains);
                        continue;
                    case "value":
                        xpathExpression += FindExtensions.GetAttributeXPath("value", value, isContains);
                        continue;
                    case "name":
                        xpathExpression += FindExtensions.GetAttributeXPath("name", value, isContains);
                        continue;
                    case "text":
                    case "innertext":
                        ...
                        continue;
                    case "type":
                        xpathExpression += FindExtensions.GetAttributeXPath("type", value, isContains);
                        continue;
                    case "href":
                        xpathExpression += (isContains
                            ? "[contains(@href, \"" + value + "\")]"
    ...
    FindExtensions.GetAttributeXPath = function (attribute, value, isContains) {
        return isContains ? "[contains(@" + attribute + ", '" + value + "')]" : "[@" + attribute + "='" + value + "']";
    };
    FindExtensions.FindSubElement = function (baseObject, elementProperties) {
        return baseObject.findElement(this.BuldUniversalByClause(elementProperties))
            .then(function (value) { return new SeleniumWebElement(value); });
    };
    FindExtensions.FindSubElements = function (baseObject, elementProperties) {
        return (baseObject.findElements(this.BuldUniversalByClause(elementProperties))
            .then(function (results) { return (results.length < 1 ? [] : results.map(function (e) { return new SeleniumWebElement(e); })); }));
    };
    FindExtensions.BuldUniversalByClause = function (elementProperties) {
        var keys = [];
        ...
        if (keys.length === 1) {
            return FindExtensions.BuildBy(keys[0], elementProperties[keys[0]]);
        }
        else {
            return FindExtensions.BuildCompositeXPathBy(elementProperties);
        }
    };
