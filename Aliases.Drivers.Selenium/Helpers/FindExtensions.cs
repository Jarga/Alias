...
namespace Aliases.Drivers.Selenium.Helpers
{
    public static class FindExtensions
    {
        ...
        public static IList<ITestableWebElement> FindSubElements(this ISearchContext baseObject, IDictionary<string, string> elementProperties)
        {
            ...
            {
                string key = properties.Keys.ElementAt(0);
                elements = baseObject.FindElements(BuildBy(key, properties[key]));
            }
            else
            {
                elements = baseObject.FindElements(BuildCompositeXPathBy(properties));
            }
